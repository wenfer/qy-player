...
- **跳过片头/片尾**：Jellyfin 自动识别片头段自动跳过；剧集详情页可
  自定义整剧片头/片尾秒数（Emby 也可用）
- **统一首页**：本地、WebDAV、Jellyfin、Emby 的继续观看 / 最近添加 /
  搜索合并去重（按完整来源身份，不错归属）
- **系统托盘**：运行中可从托盘快速显示/隐藏窗口；关闭窗口即退出应用
- **全局快捷键**：多媒体键控制播放
- **音乐播放（三期）**：本地 / WebDAV 音频扫描入库（自研标签解析、
  内嵌封面、CUE 分轨），服务器（Jellyfin/Emby）音乐库可直接浏览播放；
  双播放引擎（常见格式走内置引擎拿实时频谱，冷门格式与服务器音频走 mpv
  保证兼容）；歌单（含 m3u/m3u8 导入、XSPF 导出）；
  桌面歌词浮窗（透明置顶、鼠标穿透、逐字高亮，服务器曲目同样支持）；
  歌词面板与 .lrc 导入；均衡器 / ReplayGain / 拾音器
