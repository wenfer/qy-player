...
3. **歌单**：内部歌单 + m3u8 / XSPF 导出（m3u 导入于 1.5.0 移除，见 QYP3-068k）。
...
- 导出：m3u8（本地项相对路径）+ XSPF（全部来源，URL 化）；导入侧
  （m3u/m3u8）已于 1.5.0 移除（QYP3-068k）。
- 导出只涉及**本地与 WebDAV 项**；Jellyfin/Emby 服务器内建歌单三期
  只读展示（P2）。
...
| 016 | m3u8 导出（相对路径；导入侧 1.5.0 移除） | 015 |
...
3. **桌面歌词在无合成器 WM 上黑底**：降级路径写进验收（不透明窗口 +
   提示），不做"假装透明"。
4. **离线频谱 spike 可能失败**：【QYP3-050 已落地，红线已改】原计划是失败则
   mpv 引擎降级为播放波形、**不引入 ffmpeg 依赖**；实测 mpv 0.32 的编码模式是
   坏的（`--o=` 每帧 `error encoding`，拿不了 PCM），所以改成**可选依赖**：
   运行时探测 `~/.local/bin/ffmpeg` → PATH，找不到就静默降级为静音底线。
   声音路径完全不动（仍由 mpv 播放），预算结果落 `userData/music-spectrum/`
   （见 OPERATIONS §5）。
5. **老机 CPU**：拾音器默认帧率 30fps；AnalyserNode fftSize ≤ 2048；
   离线频谱解码并发=1、起播 3s 后才开工、硬超时 90s、>2 小时曲目跳过；
   可在设置 → 音乐整体关闭。
