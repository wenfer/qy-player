...
interface SourceFormProps {
  ...
  /**
   * 用途由所在模式的媒体库页决定（QYP3-041）：影视模式建视频来源、
   * 音乐模式建音乐来源。不支持音乐与视频混放在同一目录，故不再给用户选。
   */
  purpose: SourcePurpose;
...
export default function SourceForm({
  ...
  return (
    <div className="mb-4 p-4 bg-card border border-border rounded-xl space-y-3" data-testid="source-form">
      ...
      {/* 用途（QYP3-041）：由所在模式决定，音乐源只索引音频，视频源只索引视频 */}
      <p className="text-[11px] text-muted-foreground">
        {purpose === 'music' ? '作为音乐来源：扫描时只索引音频与分轨文件' : '作为影视来源：扫描时按视频与 NFO 归类，不索引音频'}
      </p>
