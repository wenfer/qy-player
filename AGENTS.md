...
### UI 约定（用户偏好，勿回退）
- **应用顶层双模式**（QYP3-040）：视频模式（默认，`stores/app-mode-store`）与音乐模式完全隔离——侧栏「影视/音乐」分段切换，切换即导航到该模式默认页（`/` 或 `/music`）；导航项、设置页签、媒体库入口随模式整组更换（音乐媒体库在 `/music-sources`）。迷你条/精简浮窗**跨模式保留**（由音乐会话门禁，非页面门禁）；直达 hash 路由不做模式推断（只控制可见入口）。模式不持久化：每次启动都是视频模式
- **设置页仅软件配置**（服务器、快捷键等）；媒体来源按模式拆分管理（QYP3-039/040）：视频模式「媒体库」（`/media-sources`）管服务器与视频来源、音乐模式「媒体库」（`/music-sources`）管音乐来源；来源带 `purpose` 标记（`all/music/video`，扫描按用途过滤，收窄用途会清理另一域索引），勿合并回单一页面
