...
### QYP3-068m 音乐页去大标题 + 右边距收紧 `[x]`
- 用户：曲目列表上方的大字「音乐」没作用，移除；列表右边距稍大
- 改动（pages/Music）：
  ① 可见 h1 删除，保留 `sr-only` 的 h1「音乐」（AGENTS.md 标题层级约定，
  音乐模式顶部工具条已有可见的「音乐」页签）
  ② 页面容器 `p-4` → `pl-4 pr-2 pt-3 pb-4`——窄屏里滚动条占掉右侧 ~8px，
  两侧等宽内边距会显得右边空；右侧 28px → 24px（到收藏图标边缘）
- 验证：CDP 实测行左 16px / 心形图标右 24px、无横向溢出、200 行渲染正常；
  typecheck + tests/renderer/music 137 例全绿

### QYP3-068n 音乐模式移除最大化按钮 `[x]`
- 用户：音乐模式不需要最大化
- 改动（`components/TitleBar`）：音乐模式（非浮窗）不渲染最大化按钮，双击
  标题栏也不再触发最大化——380×740 的竖屏铺满屏幕没有意义。**浮窗的
  「还原窗口」保留**（profile 优先级更高，那是唯一的逃生口）
- 验证：CDP 实测音乐模式标题栏只剩「最小化 / 关闭」，影视模式仍是「最大化 /
...
### QYP3-068r 频谱改老式功放 LED 观感（配色 + 密度 + 自动量程） `[x]`
- 用户：浮窗频谱"有点太密集"，配色想更接近老式功放的 LED、经典风格
  → 选择「绿→黄→红 三段分区」+「减柱子 + 压缩电平」
- 实测（CDP 拉满 analyser 量程，真机播放本地曲目采 40 帧 × 64 带）：
  未接来电(Live) mp3 电平 p50 0.58 / max 0.90；以父之名 flac p50 0.27 /
  max 0.51——**同一批文件响度差 ~10dB**，固定刻度救不了（按响的调→安静曲目
  只剩几格；按安静调→响的曲目顶成一片）。
  APE 走 mpv 离线频谱这条路径在本机**拿不到数据**（无 ffmpeg 预算结果），
  所以只按 webaudio 路径定标
- 改动（全在 `Visualizer/bars-painter.ts`，两处画图方各只改柱数）：
  ①`LED_GREEN/LED_AMBER/LED_RED` 三段分区（60/25/15）+ `segmentColor()`，
  峰值帽改中性亮白；②柱数 56→40（频谱图）/ 48→32（拾音器），段间距
  0.28→0.34；③`shapeLevel()` 扣噪声底 + γ=2 曲线，`paint` 里先做**自动量程**
  （参考电平立刻跟峰值、2.5s 慢回落、下限 0.25）；
  ④`createSpectrumDecay` 默认 τ 150→450ms（衰减作用在压缩前的字节上）
- 验证：CDP 真机播放两首（响 mp3 / 安静 flac）截图目视——两首都占满面板、
  绿底/琥珀中段/红顶清晰、柱间有间隙、峰值帽可见；暂停回落仍平滑；
  typecheck 双配置 + test:render 全绿（新增 AGC 与三段分区用例）
- **踩坑**：最初按"数据被削顶"假设做了 γ=2.2 + 噪声底 0.12，实测发现
  数据根本没削顶（0% 到 255），那样调会把安静曲目压成一排黑、红灯永不亮
- 追加（QYP3-068s，用户反馈"浮窗的频谱还是密集"）：上一轮只改了配色与柱数，
  **浮窗那块的几何没动**——浮窗是固定 400×300 不可缩放，频谱卡片 `flex-1`
  把剩余高度全吃掉，画布 374×134 / 16 段 → 每格 6.4×6 正方形。现在
  `COMPACT_HEIGHT` 300→240，画布 ≈374×74，格子变 6.4×3 横向灯珠
